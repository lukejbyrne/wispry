...
resolve_signing() {
  ...
  local_id="$(find_identity "IDontType Local Code Signing")"
  if [[ -z "$local_id" ]]; then
  ...
  case "$SIGN_MODE" in
    ...
    local)
      ...
      if [[ -z "$SIGN_IDENTITY" ]]; then
        echo "error: SIGN_MODE=local requested, but no IDontType/i don't type/Wispry local signing identity is installed." >&2
        exit 1
      fi
      RESOLVED_SIGN_MODE="local"
      ;;
...
if [[ "$RESOLVED_SIGN_MODE" == "adhoc" ]]; then
  echo "warning: no Developer ID, Apple Development, or IDontType local signing identity was found; built with ad-hoc signing." >&2
fi
